...
This dApp uses wallet signatures as the source of identity. Supabase provides Auth, client APIs, and the platform runtime, while Drizzle owns the app tables for users, challenges, and sessions.
...
- Supabase stores platform-managed state and client-facing API access, while the browser never receives the service-role key.
...
Drizzle owns the table definitions, indexes, foreign keys, and check constraints for those tables. Supabase migrations handle the custom SQL pieces that Drizzle should not own:
...
- The Supabase CLI runs the local stack with `supabase start`, `supabase stop`, and `supabase db reset`.

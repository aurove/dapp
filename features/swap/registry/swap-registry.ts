...
export function getSwapRoutingConfig(): SwapRoutingConfig {
  return {
    ...
    maxCandidateRoutes: boundedInteger(process.env.NEXT_PUBLIC_SWAP_MAX_CANDIDATE_ROUTES, 8, 1, 64),
...
function poolCandidates(chainId: number) {
  ...
  return Object.entries(contracts).flatMap(([key, contract]) => {
    if (
      !contract.address ||
      !hasFunction(contract.abi as Abi, "token0") ||
      !hasFunction(contract.abi as Abi, "token1") ||
      !hasFunction(contract.abi as Abi, "tickSpacing") ||
      !hasFunction(contract.abi as Abi, "slot0")
    )
      return [];
...
export function getSwapAssets(chainId: number): SwapAsset[] {
  ...
  return known.map((token) => ({
    id: `erc20:${token.symbol}`,
    chainId,
    address: token.address,
    executableAddress: token.address,
    symbol: token.symbol,
    name: token.symbol === "MUSD" ? "Mezo USD" : token.symbol,
    decimals: token.decimals,
    form: "erc20",
    balanceDomain: "wallet",
    balanceKey: token.symbol,
  }));
}
...
function mergeManagedWrappers(
  primary: ManagedWrapper[],
  fallback: ManagedWrapper[],
): ManagedWrapper[] {
...
export async function loadSwapRegistry(
  client: PublicClient,
  chainId: number,
): Promise<SwapRegistry> {
  ...
  if (
    !clRouter?.address ||
    !auroveRouter?.address ||
    !ledger?.address ||
    !clFactory?.address ||
    !id20Factory?.address
  )
    throw new Error("Swap routers are not deployed");
  ...
  const poolCount = await client.readContract({
    address: clFactory.address,
    abi: clFactory.abi,
    functionName: "allPoolsLength",
  });
  if (typeof poolCount !== "bigint" || poolCount > BigInt(Number.MAX_SAFE_INTEGER))
    throw new Error("Pool registry returned an invalid length");
  const poolAddressResults = (await client.multicall({
    allowFailure: true,
    contracts: Array.from({ length: Number(poolCount) }, (_, index) => ({
      address: clFactory.address!,
      abi: clFactory.abi,
  ...
  const poolAddresses = poolAddressResults.flatMap((result) =>
  ...
  const pools: SwapPool[] = poolAddresses.flatMap((address, index) => {
    ...
    const [token0, token1, tickSpacing, fee] = values.map((item) =>
    ...
    if (
      ...
      return [];
    const configured = configuredPools.find(
      (candidate) => candidate.contract.address.toLowerCase() === address.toLowerCase(),
    );
    return [
      {
        key: configured?.key ?? `cl:${address.toLowerCase()}`,
        address,
        abi: poolAbi,
  ...
  const wrapperResults = (await client.multicall({
    allowFailure: true,
    contracts: trancheDefinitions.map((tranche) => ({
      address: id20Factory.address!,
      abi: id20Factory.abi,
  ...
  const wrapperAssets: SwapAsset[] = deployedWrappers.map((wrapper) => ({
    id: `id20:${wrapper.address.toLowerCase()}`,
    chainId,
    address: wrapper.address,
    executableAddress: wrapper.address,
    symbol: symbolOf(wrapper.variant, wrapper.epochs),
    name: nameOf(wrapper.variant, wrapper.epochs),
    decimals: 18,
    form: "id20",
    balanceDomain: "id20",
    balanceKey: `id20:${wrapper.address.toLowerCase()}`,
  ...
  const trancheAssets: SwapAsset[] = deployedWrappers.map((wrapper) => ({
    id: `tranche:${wrapper.trancheId}`,
    chainId,
    address: ledger.address!,
    executableAddress: wrapper.address,
    symbol: symbolOf(wrapper.variant, wrapper.epochs),
    name: `${nameOf(wrapper.variant, wrapper.epochs)} tranche`,
    decimals: 18,
    form: "tranche",
    balanceDomain: "tranches",
    balanceKey: `tranche:${wrapper.trancheId}`,
    trancheId: wrapper.trancheId,
    variant: wrapper.variantId,
    epochs: BigInt(wrapper.epochs),
    wrapperAddress: wrapper.address,
  ...
  const poolTokens = [
    ...new Set(pools.flatMap((pool) => [pool.token0.toLowerCase(), pool.token1.toLowerCase()])),
  ] as Address[];
  const routableAssets = [...configuredAssets, ...wrapperAssets];
  const missingTokens = poolTokens.filter(
    (address) => !routableAssets.some((asset) => asset.executableAddress.toLowerCase() === address),
  );
  const metadata = (await client.multicall({
    allowFailure: true,
    contracts: missingTokens.flatMap((address) => [
  ...
  const discoveredAssets: SwapAsset[] = missingTokens.flatMap((address, index) => {
    ...
    if (typeof symbol !== "string" || typeof name !== "string" || typeof decimals !== "number")
      return [];
    const canonicalWalletAsset = configuredAssets.find(
      (asset) =>
        asset.balanceDomain === "wallet" && asset.address.toLowerCase() === address.toLowerCase(),
    );
    return [
      {
        id: `erc20:${address.toLowerCase()}`,
        chainId,
        address,
  ...
  const knownTokens = getKnownMezoTokenConfigs(chainId);
  const basicProbePairs = knownTokens.flatMap((left, index) =>
    knownTokens.slice(index + 1).flatMap((right) => [
      { tokenA: left.address, tokenB: right.address, stable: false },
      { tokenA: left.address, tokenB: right.address, stable: true },
    ]),
  );
  const basicPoolResults = amm
    ? ((await client.multicall({
        allowFailure: true,
        contracts: basicProbePairs.map((pair) => ({
          address: amm.router,
          abi: MEZO_BASIC_ROUTER_ABI,
          functionName: "poolFor",
          args: [pair.tokenA, pair.tokenB, pair.stable, amm.poolFactory],
        })),
      })) as ReadResult[])
    : [];
  const basicPools: SwapBasicPool[] = amm
    ? basicProbePairs.flatMap((pair, index) => {
        const result = basicPoolResults[index];
        const address =
          result?.status === "success" && typeof result.result === "string"
            ? (result.result as Address)
            : zeroAddress;
        ...
        const [token0, token1] =
          pair.tokenA.toLowerCase() < pair.tokenB.toLowerCase()
            ? [pair.tokenA, pair.tokenB]
            : [pair.tokenB, pair.tokenA];
        return [
          {
            key: `basic:${address.toLowerCase()}:${pair.stable ? "stable" : "volatile"}`,
            address,
            token0,
            token1,
            stable: pair.stable,
            factory: amm.poolFactory,
          },
        ];
      })
    : [];
  return {
    chainId,
    revision: `${portfolio?.revision ?? ""}:${pools.map((pool) => `${pool.address}:${pool.tickSpacing}:${pool.fee}`).join("|")}:${basicPools.map((pool) => pool.address).join("|")}`,
...
export function withWalletVeNfts(
  registry: SwapRegistry | undefined,
  wallet: WalletPortfolio | undefined,
): SwapRegistry | undefined {

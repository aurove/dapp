...
import { canSwapRoute } from "../routing";
...
function compareSelectorAssets(a: SwapAsset, b: SwapAsset, side: "sell" | "buy"): number {
  ...
  const variantDifference =
    (a.variant ?? Number.MAX_SAFE_INTEGER) - (b.variant ?? Number.MAX_SAFE_INTEGER);
  if ((a.form === "venft" || a.form === "tranche") && variantDifference !== 0)
  ...
  if (a.form === "venft" && b.form === "venft" && a.tokenId !== b.tokenId)
    return (a.tokenId ?? 0n) < (b.tokenId ?? 0n) ? -1 : 1;
...
export function useSwapAssets(
  ...
  opposite: SwapAsset | undefined,
  side: "sell" | "buy",
) {
  const portfolio = usePortfolioSummary();
  const { address } = useAccount();
  const client = usePublicClient();
  const dynamicFungibleAssets = useMemo(
    () =>
      registry?.assets.filter(
        (asset) => asset.form === "underlying" || asset.form === "erc20" || asset.form === "id20",
      ) ?? [],
    [registry],
  );
  const trancheAssets = useMemo(
    () => registry?.assets.filter((asset) => asset.form === "tranche") ?? [],
    [registry],
  );
  const discoveredBalances = useQuery({
    ...
    queryFn: async () => {
      ...
      const results = await client.multicall({
        allowFailure: true,
        contracts: [
          ...dynamicFungibleAssets.map((asset) => ({
            address: asset.address,
            abi: erc20Abi,
            functionName: "balanceOf",
            args: [address],
          })),
          ...trancheAssets.map((asset) => ({
            address: registry!.ledger.address,
            abi: registry!.ledger.abi,
            functionName: "balanceOf",
            args: [address, asset.trancheId!],
          })),
        ],
      });
    ...
    enabled: Boolean(
      client && address && registry && (dynamicFungibleAssets.length || trancheAssets.length),
    ),
    staleTime: 15_000,
  });
  const assets = useMemo(() => {
    ...
    return registry.assets
      .filter((asset) => {
        ...
        return canSwapRoute(registry, tokenIn, tokenOut);
      })
  ...
  const balanceOf = (asset: SwapAsset): bigint => {
    ...
    if (asset.form === "tranche" && asset.trancheId !== undefined) {
      const tranche = Object.values(portfolio.data?.trancheBalances ?? {}).find(
        (item) => item.trancheId === asset.trancheId,
      );
      if (tranche) return tranche.rawBalance;
    }
    if (asset.form === "id20") {
      const id20 = Object.values(portfolio.data?.id20Balances ?? {}).find(
    ...
    if (asset.form === "erc20" || asset.form === "underlying") {
      const walletAsset = Object.values(portfolio.data?.walletAssets ?? {}).find(
        (item) => item.address.toLowerCase() === asset.address.toLowerCase(),
      );
    ...
    if (asset.balanceDomain === "wallet")
      return portfolio.data?.walletAssets[asset.balanceKey]?.rawBalance ?? 0n;
    if (asset.balanceDomain === "tranches")
      return portfolio.data?.trancheBalances[asset.balanceKey]?.rawBalance ?? 0n;
  ...
  return {
    ...
    balanceOf,
    isLoading:
      portfolio.domains.wallet.isLoading ||
      portfolio.domains.tranches.isLoading ||
      portfolio.domains.id20.isLoading ||
      discoveredBalances.isLoading,
  };
}

...
export async function readLiquidityPortfolio(client: PublicClient, chainId: number, owner: Address, registry: PortfolioRegistry): Promise<LiquidityPortfolio> {
  ...
  decoded.forEach((item, index) => { const poolResult = poolResults[index]; const pool = poolResult?.status === "success" && typeof poolResult.result === "string" ? poolResult.result as Address : "0x0000000000000000000000000000000000000000" as Address; if (registry.factory && poolResult?.status !== "success") failures.push(failure(`${item.tokenId}-pool`, registry.factory.address, "getPool", poolResult)); positions[item.tokenId.toString()] = { tokenId: item.tokenId, pool, token0: item.token0, token1: item.token1, tickLower: Number(item.values[5]), tickUpper: Number(item.values[6]), liquidity: item.values[7] as bigint, tokensOwed0: item.values[10] as bigint, tokensOwed1: item.values[11] as bigint }; });
  return { meta: meta(chainId, owner, blockNumber, failures), positionIds, positions };
}

...
import {
  getAuroveSupportedPools,
  type AuroveLiquidityGaugeContractName,
  type AuroveSupportedPoolKey,
} from "@/lib/config/supported-liquidity-pools";
import type { PortfolioRegistry } from "./types";

const CL_GAUGE_BY_POOL: Record<AuroveSupportedPoolKey, AuroveLiquidityGaugeContractName> = {
...
export function getPortfolioRegistry(chainId: number): PortfolioRegistry | null {
  ...
  const tranches = variants.map((variant, index) => {
    ...
    return {
      ...
      variant: index + 1,
      epochs,
      symbol: symbolOf(variant, epochs),
    };
  });
  const wrapperEntries = [
    {
      key: "avBTCm",
      name: "avBTCmId20",
      tranche: tranches.find((item) => item.key === `veBTC:${MAX_EPOCHS_BY_VARIANT.veBTC}`),
    },
    {
      key: "avMEZOm",
      name: "avMEZOmId20",
      tranche: tranches.find((item) => item.key === `veMEZO:${MAX_EPOCHS_BY_VARIANT.veMEZO}`),
    },
  ] as const;
  const id20s = wrapperEntries.flatMap(({ key, name, tranche }) => {
    const contract = getContractConfig(chainId, name);
    return contract?.address && tranche
      ? [
          {
            key,
  ...
  const walletAssets = getKnownMezoTokenConfigs(chainId).map((token) => ({
    id: token.symbol,
    ...token,
    type: "erc20" as const,
  }));
  const rewardSources = [
    {
      key: "avBTCm",
      sink: getContractConfig(chainId, "avBTCmSink")?.address,
      token: walletAssets.find((a) => a.id === "BTC"),
    },
    {
      key: "avMEZOm",
      sink: getContractConfig(chainId, "avMEZOmSink")?.address,
      token: walletAssets.find((a) => a.id === "MEZO"),
    },
  ].flatMap(({ key, sink, token }) =>
    sink && token
      ? [
          {
            key,
  ...
  const veCollections = [
    ...
    contract?.address
      ? [{ key, address: contract.address, symbol: key, abi: contract.abi as Abi }]
      : [],
  );
  const clGauges = supportedPools.flatMap((pool) => {
    ...
    if (!gaugeName) return [];
    const gauge = (
      getContractsByChainId(chainId) as unknown as Record<
        string,
        { address?: Address; abi: Abi; linkedData?: Readonly<Record<string, unknown>> }
      > | null
    )?.[gaugeName];
    if (!gauge?.address) return [];
    const linked = gauge.linkedData as { rewardToken?: string } | undefined;
    const rewardToken =
      typeof linked?.rewardToken === "string" && linked.rewardToken.startsWith("0x")
  ...
  const revisionParts = [
    ledger.address,
    ...tranches.map((item) => item.trancheId.toString()),
    ...walletAssets.map((a) => a.address),
    ...id20s.map((a) => a.address),
    ...rewardSources.map((a) => a.address),
    ...veCollections.map((a) => a.address),
    ...supportedPools.map((pool) => pool.address),
    ...clGauges.map((gauge) => gauge.address),
  ...
  return {
    ...
    positionManager: positionManager?.address
      ? { address: positionManager.address, abi: positionManager.abi as Abi }
      : undefined,

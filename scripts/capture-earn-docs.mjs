...
async function main() {
  ...
  try {
    ...
    await page.goto(`${baseURL}/earn/stake/btc`, { waitUntil: "networkidle", timeout: 60_000 });
    await page.waitForTimeout(1500);
    ...
    if (await lockTab.isVisible().catch(() => false)) {
      ...
      const amount = page
        .getByPlaceholder(/0\.0|amount/i)
        .or(page.locator('input[type="text"], input[inputmode="decimal"]'))
        .first();
    ...
    const createCta = page
      .getByRole("button", { name: /create a liquid position|approve|deposit/i })
      .first();

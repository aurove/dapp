...
The app now also includes a production-grade wallet authentication flow backed by Supabase. Wallet connection is only the first step; the user must sign a server-issued challenge before the app creates a secure session.
...
The dapp still owns its schema migrations in `supabase/migrations/20260615000100_wallet_auth.sql` and its generated database types in `lib/supabase/database.types.ts`.

For local development, the dapp Supabase values are populated into `dapp/.env.local` by `pnpm supabase:status` from the workspace root, and the app reads that file directly through Next.js env loading.
